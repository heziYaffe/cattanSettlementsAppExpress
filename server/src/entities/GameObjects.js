const TwoKeyHashMap = require('../entities/map/TwoKeyHashMap');
...
function initialTiles() {
    let tiles = new TwoKeyHashMap();

    const Tile = require('./map/Tile');
    const Hexagon = require('./map/Hexagon');
    ...
    if (tiles.isEmpty()) {
        ...
        for (let i = 0; i < 4; i++) {
            const qAndR = getQAndR(h_id);
            const q = qAndR.first
            const r = qAndR.second
            let resourceName = getRandomSeaHarbor();
            ...
            tiles.put(h_id, qAndR, new Tile(h, resourceName, 0, h_id));
            h_id++;
        }

        for (let row = 0; row < numRows; row++) {
            for (let col = 0; col < tilesPerRow[row]; col++) {
                let qAndR = getQAndR(h_id);
                const q = qAndR.first
                const r = qAndR.second
                let h = new Hexagon(q, r, h_id);
                ...
                if (col === 0) {
                    ...
                    tiles.put(h_id, qAndR, new Tile(h, resourceName, number, h_id));
                }
                h_id++;
            }
